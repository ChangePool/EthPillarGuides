...
EthPillar is an open-source tool for managing Ethereum nodes and validators through a text-based user interface (TUI). EthPillar simplifies the process of running Ethereum nodes and participating in staking.
...
<Tabs>
  <TabItem value="tui" label="TUI Update" default>
    ...
    Open EthPillar, then navigate to **System Administration > Update EthPillar**

    When the update completes, quit and then relaunch.
...
Using a terminal window, clean up the remaining files
...
:white_check_mark: **To avoid slashing penalties**, ensure that your validator keys are deleted from existing staking setups.
...
Actual steps vary depending on your computer's BIOS. For general instructions, see the topic [How to Set Up Your Computer for Auto Power On After Power Outages](https://www.wintips.org/setup-computer-to-auto-power-on-after-power-outage/).
...
Navigate to **EthPillar > Validator Client > Generate / Import Validator Keys**

Then, select one of the following options:

* Import validator keys from offline key generation or backup
* Add new or regenerate existing validator keys using a Secret Recovery Phrase
...
**What**: A failover staking node is made up of an execution client, consensus client and MEV-Boost.

**How to**: To configure for Nimbus validator client, edit your validator client configuration. For more details, see the topic [Multiple Beacon Nodes](https://nimbus.guide/validator-client-options.html#multiple-beacon-nodes).

Also, you need to expose the consensus client RPC port. Adjust your firewall to allow traffic from your validator client's IP address as well.

**Benefit**: Running multiple failover staking nodes (also known as beacon nodes) allows you to perform maintenance or have an outage on 1 failover staking node.
...
Yes. You need to manually edit the `deposit_data` JSON for each activation.
...
2. System resources (CPU, RAM, disk space) and port accessibility using **Security & Node Checks > Node Checker**
...
EthPillar includes:
- Go to **Security & Node Checks > Node Checker**
- [Dashboarding](/Monitoring) using **Logs and Monitoring**
...
Yes, you can configure remote access to Grafana dashboards. However, EthPillar recommends using SSH tunneling for security.
...
To exit, press `Ctrl+B`, and then press `D` This is a tmux command. For different layouts, press `Ctrl+B`, and then press `SPACE`
